    ...
    <div class="container mx-auto max-w-6xl" :class="{ 'mt-12': !connected }">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Button
          v-for="key in keys"
          :key="key.id"
          :variant="key.status === 'OPENED' || key.status === 'OPENING' ? 'destructive' : 'outline'"
          @click="toggleKey(key)"
          :disabled="!connected"
          class="h-16"
        >
      ...
      <div class="space-y-2 mb-6">
        ...
        >
          <div
            ...
            :style="{
              width: key.status === 'OPENED' || key.status === 'OPENING' ? '100%' : '0%',
          ...
          <div v-if="isProgramKey(key)" class="space-y-4">
            ...
            <div v-if="getCurrentProgram(key)">
              ...
              <input
                ...
                @input="updateProgramField(getCurrentProgram(key)!, 'num', ($event.target as HTMLInputElement).value)"
                :disabled="!connected"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-400"
              />
            </div>
            ...
            <div v-if="getCurrentProgram(key)">
              ...
              <input
                ...
                @input="updateProgramField(getCurrentProgram(key)!, 'person', ($event.target as HTMLInputElement).value)"
                :disabled="!connected"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-400"
              />
            </div>
            ...
            <div v-if="getCurrentProgram(key)">
              ...
              <input
                ...
                @input="updateProgramField(getCurrentProgram(key)!, 'name', ($event.target as HTMLInputElement).value)"
                :disabled="!connected"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-400"
              />
            </div>
          ...
          <div v-else-if="isLyricsKey(key)" class="space-y-4">
            <div>
              ...
              <select
                ...
                @change="selectSong(key, Number(($event.target as HTMLSelectElement).value))"
                :disabled="!connected"
                class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-gray-400"
              >
                <option
              ...
              <div class="border border-gray-300 rounded-md max-h-60 overflow-y-auto">
                <div
                  ...
                  :class="[
                    ...
                    getCurrentSong(key)!.current_lyric_id === lyric.id ? 'bg-gray-100' : ''
                  ]"
                >
                  {{ lyric.text || '(空行)' }}
                </div>
            ...
            <div class="flex gap-2">
              ...
              <Button @click="lyricsForward(key)" :disabled="!connected" size="sm">
                前进
              </Button>
              <Button @click="lyricsPlayForward(key)" :disabled="!connected" size="sm">
                播放前进
              </Button>
            </div>
          </div>
...
async function updateKeyTransition(key: Key, event: Event) {
  ...
  try {
    await api.updateKey(key.id, { transition_time: value, version: key.version })
    key.version++
  } catch (error) {
...
async function updateProgramField(program: ProgramData, field: keyof ProgramData, value: string) {
  if (!connected.value) return
...
async function selectSong(key: Key, songId: number) {
  ...
  try {
    await api.updateKey(key.id, { current_preset_id: songId, version: key.version })
    key.version++
  } catch (error) {
...
async function lyricsBack(key: Key) {
  ...
  const currentIndex = song.lyrics.findIndex(l => l.id === song.current_lyric_id)
  if (currentIndex > 0) {
...
async function lyricsPlayForward(key: Key) {
  ...
  const song = getCurrentSong(key)
  if (song && song.lyrics) {
    const currentLyric = song.lyrics.find(l => l.id === song.current_lyric_id)
    if (currentLyric) {
